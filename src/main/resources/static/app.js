...
function connect() {
    ...
    socket = new SockJS('/sample-stomp-chat');
...
function disconnect() {
    if (stompClient !== null) {
        $("#conversation").append("<tr><td>" + shortName + " just left</td></tr>");
...
function sendName() {
    stompClient.send("/app/guestjoin", {}, JSON.stringify({ 'message': $("#shortName").val(), 'senderName': $("#message").val(), 'senderEmail': $("#senderEmail").val(), 'senderMobileNo': $("#senderMobileNo").val() }));
    $("#profileName").html($("#shortName").val());
}

function showJoinedName(message) {
    shortName = message;
    $("#chatMessages").append("<tr><td>" + message + " just joined</td></tr>");
}
...
$(function () {
    ...
    $("#clearChat").click(function () { clearChatMessages(); });

    $("#message").keyup(function (e) {
        // Send "is typing" message to server after keystrokes detected
        stompClient.send("/app/guestupdate", {}, JSON.stringify({ 'message': $("#message").val(), 'senderName': $("#shortName").val() }));
    });
});
